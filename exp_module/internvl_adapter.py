...
class InternVLAdapter(StreamModelAdapter):
    ...
    def lm_prefill(self, model, **kwargs):
        # prefill 只需要 past_key_values，不需要 logits。直接呼叫底層
        # base model（Qwen2ForCausalLM.model）跳過 lm_head，省掉
        # (B, chunk_size, vocab) 這個算完馬上被丟掉的巨大暫時張量
        # （B=4 / chunk=2048 / vocab≈152k / bf16 ≈ 2.5 GB，實測是 injection
        # 階段 peak 的主因）。base model 的輸出一樣帶 .past_key_values。
    ...
    def encode_and_bank(
        ...
        _merge_extra = (f"  merge_weight={merge_weight}"
                        if merge_mode in ("merge", "merge_spatial") else "")
        print(f"\n[Memory Bank] budget={budget}  score_fn={score_fn}  mode={merge_mode}  "
              f"select={select}  tile_agg={tile_agg}{_merge_extra}")
        print(f"  tiles per image (before compression) : {num_tiles_list}")
        print(f"  raw vision tokens per image          : {[n * num_image_token for n in num_tiles_list]}")
        ...
        for b in range(B):
            ...
            if merge_mode in ("merge", "merge_spatial"):
                print(f"           {merge_mode}: {stats['tokens_merged']} tok folded, "
                      f"{stats['tokens_hard_dropped']} hard-dropped, "
                      f"mean_merge_sim={stats['mean_merge_sim']:.3f}")
