...
# 全域變數，確保 Profiler 只在 Warmup 結束後才啟動
ENABLE_PROFILING = False
# 確保 Prefill 只錄製一次，避免錄到後續 Decode 階段的碎小 Kernel
PREFILL_DONE = False
...
def register_profiler_hooks(model, target):
    """
    動態掛載 CUDA Profiler API。
    透過攔截 forward 函數，精準控制 NCU 的錄影開關。
    """
    core = model.model

    if target == "vision" and hasattr(core, "vision_tower"):
        original_forward = core.vision_tower.forward

        @functools.wraps(original_forward)
        def vision_wrapper(*args, **kwargs):
            if ENABLE_PROFILING:
                torch.cuda.synchronize()
                torch.cuda.profiler.start() # 🔴 開啟硬體錄製
            try:
                res = original_forward(*args, **kwargs)
                if ENABLE_PROFILING:
                    torch.cuda.synchronize()
                ...
                if ENABLE_PROFILING:
                    torch.cuda.profiler.stop() # ⏹ 關閉硬體錄製

        core.vision_tower.forward = vision_wrapper
        print("✅ 已成功掛載 CUDA Profiler API 於: Vision_Tower")

    elif target == "prefill" and hasattr(core, "language_model"):
        original_forward = core.language_model.forward
        
        @functools.wraps(original_forward)
        def llm_wrapper(*args, **kwargs):
            global PREFILL_DONE
            # 只在 ENABLE_PROFILING 為 True 且尚未完成 Prefill 時才錄製
            should_profile = ENABLE_PROFILING and not PREFILL_DONE
            ...
            try:
                ...
                if should_profile:
                    torch.cuda.profiler.stop() # ⏹ 關閉硬體錄製
                    PREFILL_DONE = True # 標記已錄完 Prefill，放棄後續 Decode
                    
        core.language_model.forward = llm_wrapper
        print("✅ 已成功掛載 CUDA Profiler API 於: LLM_Forward (僅限 Prefill 階段)")
...
def main():
    ...
    parser.add_argument("--target", type=str, choices=["vision", "prefill", "none"], default="vision", help="選擇要精準分析的目標區段")
    ...
    # 註冊 NVTX Hooks
    if args.target != "none":
        register_profiler_hooks(model, args.target)
    ...
    times = []
    print(f"Starting Profile Region for Target: {args.target.upper()}...")
    ENABLE_PROFILING = True # 允許 Profiler 開始作動
