...
def run_online_vision_kv_pipeline(model, processor, inputs, messages, texts, dtype=torch.float16, chunk_size=1024, vit_batch=5):
    # ==========================================
    ...
    with torch.no_grad():
        ...
        print(f"-> Compression complete, final packed_image_features = {packed_image_features.shape} (around {total_vision_tokens} tokens)")
    ...
    text_before, text_after = texts[0].split("<image>")
    ...
    for i in range(0, total_vision_tokens, chunk_size):
        ...
        print(
            ...
            f"memory alloc: {mem_alloc:.2f} GB"
        )
...
def main():
    ...
    # ref_answer = generate_answer(model, processor, inputs, messages)
    # print("\n" + "="*50)
    # print(ref_answer)
    # print("\n" + "="*50)

    answer = run_online_vision_kv_pipeline(model, processor, inputs, messages, texts, dtype=dtype)
    # print(f"Online Vision completed, CUDA memory allocated: {torch.cuda.memory_allocated() / 1e9:.2f} GB")
